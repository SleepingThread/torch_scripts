...
from torch_scripts.prune.random import RandomPruner
...
model.info = {
    "model": "VGG16",
    "data": "cifar10",
    "comment": "Random prune, Adam + l2(2e-5) + drop(0.2), cosine lr scheduler",
    "status": "running"
}
...
modules = [(_m, None) for _m in model.features.modules() if isinstance(_m, torch.nn.Conv2d)]
modules += [(model.classifier, None)]
pruner = RandomPruner(model, modules, opt, storage=storage)
logs = train_loop(model, loss_l2, loaders, opt, epochs, device=device, metrics=[loss_l2, ce_loss, top_1],
                  callbacks=[pruner, LRSchedulerCosine(opt), logs_writer])
